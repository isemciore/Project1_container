	...
	public:
		...
		class const_iterator : 
			...
			public:	//Låta operatorerna returna unsigproxy
				...
				const_iterator(std::size_t pointIndex):currentIndex(pointIndex){}
				~const_iterator();/*{}*/
				bool operator*() const;
			...
			private:
				const Vector<bool>& _boolData; //ta bort const hur? //Wrapper som kollar ifall const eller inte..?
				//const Vector<bool>&  _boolData;
				//Fråga hur const iterator deference kan få tillgång till "iterator" data
		};
		class iterator: public const_iterator{
			public:
				iterator(Vector<bool> & boolData,std::size_t pointIndex)
					: const_iterator(pointIndex)
					, _boolData(boolData){}
			private:
				Vector<bool>& _boolData; //Hur skriva över?
