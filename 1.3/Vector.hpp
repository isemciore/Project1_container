...
class Vector {
    ...
    public:
        ...
        virtual void insert(const std::size_t &,const T &);
        void sort(bool);
        ...
        void reset();
        void unsafeSwap(std::size_t from, std::size_t to);
};
...
template <class T>
void Vector<T>::unsafeSwap(std::size_t from, std::size_t to){
	if(from < _nActiveE && to < _nActiveE){
		T temp_1 = _dataArray[to];
		_dataArray[to] = _dataArray[from];
		_dataArray[from] = temp_1;
	}
